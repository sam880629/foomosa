...
api.get('/:id', (req, res) => {
    ...
    const sql = 
    `SELECT * FROM shop WHERE shop_id = ? ; SELECT * FROM class WHERE class_id = (SELECT class_id FROM shop WHERE shop_id = ?); SELECT * FROM menu WHERE shop_id = ?;SELECT shop_id, active_title, active_picture, DATE_FORMAT(active_editdate,'%Y/%m/%d %H:%i')active_editdate, DATE_FORMAT(active_startdate,'%Y/%m/%d')active_startdate, DATE_FORMAT(active_enddate,'%Y/%m/%d')active_enddate , active_content, active_ifDel FROM active WHERE shop_id = ?ORDER BY active_startdate DESC;SELECT * FROM user WHERE user_id = ?;`;
    dbConn.query(sql, //dbConn 啟動 sql //送出 sql 指令  
        [req.params.id, req.params.id, req.params.id, req.params.id,userId], //id 是從 URL 中獲得的參數
